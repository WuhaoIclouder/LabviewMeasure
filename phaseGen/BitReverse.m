function reversedArray = BitReverse(originalArray)
[h,w]=size(originalArray);
BitReverseTableHex =  [ ...
...
BitReverseTableHex=reshape(BitReverseTableHex',1,512);
BitReverseTableDec = zeros(1,256);
for i=1:1:256
   BitReverseTableDec(i) = hex2dec(BitReverseTableHex(i*2-1:i*2));
end
reversedArray=zeros(h,w);
for i=1:1:h
    for j=1:1:w
        reversedArray(i,j) = BitReverseTableDec(originalArray(i,j)+1);
    end
end
